...
- [ ] **Gems tab — desktop port.** Web tab shipped in the SPA: 18 PokemonType-ordered cells against the positional `save.gems.gemWallet` array, plus a forward-compat extras panel for any future 19th type. Still pending is the Tk-side grid in `pcedit_gui.py` mirroring `ShardsTab`. Schema is locked by `tests/test_schema.py::test_gem_wallet` (`len >= 18`).
- [ ] **Installed-app icon refresh (post-web-port).** Replace `assets/icon/pokeball.svg` with the same pokeball + floppy-disk design that landed as the SPA favicon, then re-run `scripts/make_icons.py` to regenerate the `.icns`, `.ico`, and PNG variants. Brand consistency between the desktop installer and the browser app.
